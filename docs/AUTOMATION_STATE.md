...
- **#17** `auto/lockdown-negative-cost-driver-coins` — **aperta da QUESTA sveglia**: scaffold
  SQL (non applicato) per il debito "Gruppo 3" annotato in HANDOFF.md 9/8 — 6 RPC Driver Coins
  (`rpc_upgrade_offline_limit`, `rpc_buy_auto_rest`, `rpc_buy_energy_refill`,
  `rpc_buy_fleet_repair`, `rpc_buy_vip_contact`, `rpc_buy_hr_automation`) validavano solo
  `driver_coins < costo` ma mai il segno → costo negativo = conio arbitrario di Driver Coins,
  stessa classe delle falle già chiuse il 9/8 ma mai estesa a questa famiglia. Verificato
  leggendo `05_mmo_driver_coins.sql` + `12_hr_automation.sql` riga per riga prima di scrivere il
  fix (non fidato del solo elenco in HANDOFF.md). Vedi https://github.com/Normally101/ncc/pull/17.

**Debito Gruppo 3 residuo, non ancora toccato da nessuno (candidati per le prossime sveglie,
vedi HANDOFF.md 9/8 per il dettaglio completo):**
...
fidarsi di questo file), leggere `HANDOFF.md` per eventuali sessioni live nel frattempo. Se
#9/#10/#15/#16/#17 sono ancora tutte aperte e nessuna novità umana → prendere UNO dei candidati
