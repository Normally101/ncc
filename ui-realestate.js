...
async function renderTabRealEstate() {
    ...
    <div style="padding:16px;max-width:800px">
        ...
        <div style="display:flex;flex-direction:column;gap:12px">
            <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:20px;text-align:center">
                <div style="font-size:11px;color:#6b7280">Caricamento immobili…</div>
            </div>
        </div>
    </div>`;
    ...
    try {
        ...
        container.innerHTML = `<div style="padding:16px;color:#f85149;font-size:12px">Errore caricamento immobili: ${e.message}</div>`;
        return;
    }
...
<div style="padding:16px;max-width:800px">
    ...
    <div style="padding-bottom:16px;border-bottom:1px solid #21262d;margin-bottom:16px">
        ...
        <div style="font-size:11px;color:#8b949e;margin-top:2px">${ownedIds.size} proprietà · Valore €${portfolioValue.toLocaleString('it-IT')} · Rendita €${totalDailyRent > 0 ? totalDailyRent.toLocaleString('it-IT') : '0'}/g</div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:20px">
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            ...
            <div style="font-size:18px;font-weight:700;color:${ownedIds.size>0?'#d4af37':'#e6edf3'};font-family:monospace">${ownedIds.size}</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Valore Port.</div>
            <div style="font-size:18px;font-weight:700;color:${portfolioValue>0?'#d4af37':'#e6edf3'};font-family:monospace">${portfolioValue>0?'€'+Math.round(portfolioValue/1000)+'k':'—'}</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Rendita/g</div>
            <div style="font-size:18px;font-weight:700;color:${totalDailyRent>0?'#3fb950':'#e6edf3'};font-family:monospace">${totalDailyRent>0?'+€'+totalDailyRent.toLocaleString('it-IT'):'—'}</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Budget</div>
            <div style="font-size:18px;font-weight:700;color:#58a6ff;font-family:monospace">€${Math.round((gameState.cash||0)/1000)}k</div>
        </div>
    </div>`;

    if (ownedIds.size > 0) {
        html += `<div style="font-size:10px;color:#3fb950;padding:10px 14px;background:#0d2116;border:1px solid #1a4731;border-radius:6px;margin-bottom:16px">Le rendite vengono accreditate automaticamente dal server ogni 24h.</div>`;
    }
    ...
    if (listings.length === 0) {
        ...
        listings.forEach(l => {
            ...
            html += `<div style="background:#161b22;border:1px solid ${isOwned?'#b8962b':'#21262d'};border-radius:6px;overflow:hidden">`;
            ...
            if (l.image_url) {
                html += `<div style="position:relative;height:120px;overflow:hidden">
                    ...
                    ${isOwned ? `<div style="position:absolute;top:8px;right:8px">${_pill('✓ TUO', '#d4af37')}</div>` : ''}
                    <div style="position:absolute;bottom:10px;left:14px">
                        <div style="font-size:14px;font-weight:700;color:#fff">${l.name}</div>
                        <div style="font-size:11px;color:#d4af37">${l.city}</div>
                    </div>
                </div>`;
            } else {
                html += `<div style="display:flex;justify-content:space-between;align-items:flex-start;padding:14px 14px 0">
                    ...
                    ${isOwned ? _pill('✓ TUO', '#d4af37') : ''}
                </div>`;
            }

            html += `<div style="padding:12px 14px">
                ${l.description ? `<div style="font-size:10px;color:#8b949e;line-height:1.5;margin-bottom:10px">${l.description}</div>` : ''}
                <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px">
                    ${_pill('+€'+(l.daily_rent||0).toLocaleString('it-IT')+'/g', '#3fb950')}
                    ${l.bonus_type === 'driver_stress_recovery' ? _pill('Recupero stress', '#a78bfa') : ''}
                    ${isOwned && nextRentStr ? _pill(nextRentStr, '#58a6ff') : ''}
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center">
                    <div style="font-size:15px;font-weight:700;color:#d4af37;font-family:monospace">€${(l.cost||0).toLocaleString('it-IT')}</div>
                    ${isOwned
                        ? _pill('✓ Rendita attiva', '#3fb950')
