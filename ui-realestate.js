...
async function renderTabRealEstate() {
    ...
    <div style="padding:16px;max-width:800px">
        <div style="padding-bottom:16px;border-bottom:1px solid #d6dee8;margin-bottom:20px">
            <div style="font-size:9px;color:#6a7480;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px">Real Estate</div>
            <div style="font-size:20px;font-weight:700;color:#1f2733">Portafoglio Immobiliare</div>
        </div>
...
<div style="padding:16px;max-width:800px">
    ...
    <div style="padding-bottom:16px;border-bottom:1px solid #d6dee8;margin-bottom:16px">
        <div style="font-size:9px;color:#6a7480;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px">Real Estate</div>
        <div style="font-size:20px;font-weight:700;color:#1f2733;letter-spacing:-.01em">Portafoglio Immobiliare</div>
        <div style="font-size:11px;color:#6a7480;margin-top:2px">${ownedIds.size} proprietà · Valore €${portfolioValue.toLocaleString('it-IT')} · Rendita €${totalDailyRent > 0 ? totalDailyRent.toLocaleString('it-IT') : '0'}/g</div>
    </div>

    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:20px">
        <div style="background:#ffffff;border:1px solid #d6dee8;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6a7480;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Proprietà</div>
            <div style="font-size:18px;font-weight:700;color:${ownedIds.size>0?'#c79a2a':'#1f2733'};font-family:monospace">${ownedIds.size}</div>
        </div>
        <div style="background:#ffffff;border:1px solid #d6dee8;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6a7480;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Valore Port.</div>
            <div style="font-size:18px;font-weight:700;color:${portfolioValue>0?'#c79a2a':'#1f2733'};font-family:monospace">${portfolioValue>0?'€'+Math.round(portfolioValue/1000)+'k':'—'}</div>
        </div>
        <div style="background:#ffffff;border:1px solid #d6dee8;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6a7480;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Rendita/g</div>
            <div style="font-size:18px;font-weight:700;color:${totalDailyRent>0?'#1aa06a':'#1f2733'};font-family:monospace">${totalDailyRent>0?'+€'+totalDailyRent.toLocaleString('it-IT'):'—'}</div>
        </div>
        <div style="background:#ffffff;border:1px solid #d6dee8;border-radius:6px;padding:10px 12px">
    ...
    if (listings.length === 0) {
        html += `<div style="text-align:center;padding:24px;background:#ffffff;border:1px solid #d6dee8;border-radius:6px;color:#6a7480;font-size:11px">Nessun immobile disponibile — il mercato si espanderà nelle prossime stagioni.</div>`;
    } else {
        ...
        listings.forEach(l => {
            ...
            html += `<div style="background:#ffffff;border:1px solid ${isOwned?'#c79a2a':'#d6dee8'};border-radius:6px;overflow:hidden">`;
            ...
            if (l.image_url) {
                ...
                html += `<div style="display:flex;justify-content:space-between;align-items:flex-start;padding:14px 14px 0">
                    <div>
                        <div style="font-size:14px;font-weight:700;color:#1f2733">${l.name}</div>
                        <div style="font-size:11px;color:#c79a2a">${l.city}</div>
                    </div>
            ...
            html += `<div style="padding:12px 14px">
                ${l.description ? `<div style="font-size:10px;color:#6a7480;line-height:1.5;margin-bottom:10px">${l.description}</div>` : ''}
