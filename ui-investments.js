...
function renderTabInvestments() {
    ...
    const _btn  = (t, fn, c, dis) => {
        const bg = c==='gold'?'#1a1608':c==='green'?'#0d2116':c==='red'?'#1e0d0d':c==='blue'?'#0d1a2e':'#161b22';
        const bd = c==='gold'?'#b8962b':c==='green'?'#1a4731':c==='red'?'#471a1a':c==='blue'?'#1e3a5f':'#21262d';
        const tc = c==='gold'?'#d4af37':c==='green'?'#3fb950':c==='red'?'#f85149':c==='blue'?'#58a6ff':'#8b949e';
...
<div style="padding:16px;max-width:800px">
    ...
    <div style="padding-bottom:16px;border-bottom:1px solid #21262d;margin-bottom:16px">
        ...
        <div style="font-size:11px;color:#8b949e;margin-top:2px">${ownedCount} / ${totalInvs} asset · Reddito passivo +€${passiveTotal.toLocaleString()}/g</div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:20px">
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            ...
            <div style="font-size:18px;font-weight:700;color:${ownedCount>0?'#3fb950':'#e6edf3'};font-family:monospace">${ownedCount}</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Reddito Passivo</div>
            <div style="font-size:18px;font-weight:700;color:#3fb950;font-family:monospace">€${passiveTotal.toLocaleString()}<span style="font-size:10px">/g</span></div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Debito Attivo</div>
            <div style="font-size:18px;font-weight:700;color:${activeLoansTotal>0?'#f85149':'#3fb950'};font-family:monospace">€${activeLoansTotal.toLocaleString()}</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Budget</div>
            <div style="font-size:18px;font-weight:700;color:#58a6ff;font-family:monospace">€${((gameState.cash||0)/1000).toFixed(0)}k</div>
        </div>
    </div>`;
        ...
        INVESTMENTS.forEach(i => {
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;padding:12px;background:#161b22;border:1px solid ${owned?'#b8962b':'#21262d'};border-radius:6px;gap:12px">
                <div style="flex:1;min-width:0">
                    <div style="font-size:12px;font-weight:700;color:${owned?'#d4af37':'#e6edf3'}">${i.name}</div>
                    <div style="font-size:10px;color:#8b949e;margin-top:3px;line-height:1.4">${i.desc}</div>
                    <div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:8px">
                        ${i.passive     ? _pill('+€'+i.passive.toLocaleString()+'/g', '#3fb950')  : ''}
                        ${i.dailyUpkeep ? _pill('−€'+i.dailyUpkeep.toLocaleString()+'/g', '#f85149') : ''}
                        ${i.buildTime   ? _pill(i.buildTime+'g build', '#f59e0b')                  : ''}
                        ${i.reqRides && !owned ? _pill(i.reqRides+' corse', reqMet?'#3fb950':'#f85149') : ''}
                    </div>
                </div>
                    ...
                    ${owned
                        ...
                        : uc
                            ? `<div style="text-align:center">
                                 <div style="font-size:11px;font-weight:700;color:#f59e0b;font-family:monospace">${daysLeft}g rimasti</div>
    ...
    if ((gameState.investments||[]).includes('inv_loan_facility')) {
        ...
        const rateColor    = dynRate <= 0.04 ? '#3fb950' : dynRate <= 0.06 ? '#f59e0b' : '#f85149';
        ...
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:14px;margin-bottom:12px">
            <div style="display:flex;justify-content:space-between;font-size:10px;color:#8b949e;margin-bottom:10px">
                <span>Debito: <span style="color:#f85149;font-weight:700;font-family:monospace">€${totalDebt.toLocaleString()}</span></span>
            ...
            <div style="height:4px;background:#21262d;border-radius:2px;overflow:hidden;margin-bottom:14px">
                <div style="height:100%;width:${debtPct}%;background:#f85149;border-radius:2px;transition:width .3s"></div>
            </div>
            <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px">
                ${[50000, 100000, 250000, 500000].map(amt => `
                <button onclick="takeLoan(${amt})" ${totalDebt >= 500000 ? 'disabled' : ''}
                    style="background:#0d1a2e;border:1px solid #1e3a5f;color:#58a6ff;padding:8px 6px;border-radius:4px;font-family:monospace;font-size:9px;font-weight:700;cursor:${totalDebt>=500000?'not-allowed':'pointer'};opacity:${totalDebt>=500000?.35:1};text-align:center">
        ...
        ${activeLoans.length > 0 ? `<div style="display:flex;flex-direction:column;gap:4px;margin-bottom:12px">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;font-size:9px;padding:6px 12px;background:#161b22;border:1px solid #21262d;border-radius:4px">
                ...
                <span style="color:#f85149;font-family:monospace">Residuo: €${l.amount.toLocaleString()} (${((l.rate||0.08)*100).toFixed(0)}%/mese)</span>
    ...
    if (vcAgencies.length > 0) {
        ...
        vcAgencies.forEach(agency => {
            ...
            const riskColor   = agency.riskLevel === 'high' ? '#f85149' : agency.riskLevel === 'medium' ? '#f59e0b' : '#3fb950';
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;padding:12px;background:#161b22;border:1px solid #21262d;border-radius:6px;gap:12px;opacity:${locked?.5:1}">
                <div style="flex:1;min-width:0">
                    ...
                    <div style="font-size:10px;color:#8b949e;margin-top:2px;line-height:1.3">${agency.desc}</div>
                    <div style="display:flex;gap:12px;margin-top:6px;font-size:9px;font-family:monospace">
                        <span style="color:#8b949e">Val: <span style="color:#e6edf3">€${(agency.valuation/1e6).toFixed(1)}M</span></span>
                        <span style="color:#8b949e">+€${agency.dailyIncome.toLocaleString()}/g</span>
                        <span style="color:${riskColor}">${agency.riskLevel.toUpperCase()}</span>
                    </div>
                    ${locked ? `<div style="font-size:9px;color:#f85149;margin-top:4px">Min. ${agency.minRep}★ · €${agency.minCash.toLocaleString()}</div>` : ''}
                </div>
                <div style="flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:6px">
                    ${stake ? `<div style="text-align:right">
                        <div style="font-size:14px;font-weight:700;color:#3fb950;font-family:monospace">${ownedPct}%</div>
                        <div style="font-size:10px;color:#3fb950;font-family:monospace">+€${dailyReturn}/g</div>
                    </div>` : ''}
    ...
    if (!holdingIncorporated) {
        ...
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:14px;margin-bottom:12px">
            ...
            <div style="font-size:10px;color:#8b949e;line-height:1.5;margin-bottom:8px">Fondare una holding ti permette di acquisire aziende sussidiarie che generano reddito passivo ogni giorno, indipendentemente dalle tue corse.</div>
            <div style="font-size:10px;color:#6b7280;margin-bottom:12px;font-family:monospace">Requisiti: <span style="color:#d4af37">4.0★</span> reputazione · <span style="color:#d4af37">€200.000</span></div>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;background:#0d2116;border:1px solid #1a4731;border-radius:6px;margin-bottom:10px">
            <div>
                ...
                <div style="font-size:10px;color:#3fb950;font-family:monospace;margin-top:2px">+€${holdingDailyIncome.toLocaleString()}/g dividendi</div>
            </div>
            <div style="font-size:10px;color:#8b949e;font-family:monospace">${(gameState.holding.subsidiaries||[]).length} sussidiarie</div>
        </div>
        ...
        ${subTemplates.map(sub => {
            ...
            return `
            <div style="display:flex;justify-content:space-between;align-items:flex-start;padding:12px;background:${owned?'#0d2116':'#161b22'};border:1px solid ${owned?'#1a4731':'#21262d'};border-radius:6px;gap:12px">
                <div style="flex:1;min-width:0">
                    <div style="font-size:12px;font-weight:700;color:${owned?'#3fb950':'#e6edf3'}">${sub.name}</div>
                    <div style="font-size:10px;color:#8b949e;margin-top:2px;line-height:1.3">${sub.desc}</div>
                    <div style="font-size:10px;color:#3fb950;font-family:monospace;margin-top:4px">+€${sub.dailyIncome.toLocaleString()}/g</div>
                </div>
