...
function renderTabMarketing() {
    ...
    if (season && season.priceMult !== 1.0) {
        html += `<div style="background:#161b228e8;border:1px solid #c79a2a;border-radius:6px;padding:10px 12px;margin-bottom:12px">
            ...
            <div style="font-size:10px;color:#e5e7eb">Tariffe +${Math.round((season.priceMult - 1) * 100)}% · Volume corse ×${season.rideBonus.toFixed(1)}</div>
        </div>`;
    }
    ...
    if (activeCampaigns.length > 0) {
        ...
        <div style="font-size:9px;color:#e5e7eb;font-style:italic;margin-bottom:10px;padding:8px;background:#0d1117;border-radius:6px">Nessuna campagna attiva. Seleziona una campagna qui sotto per avviarla.</div>`;
    }
    ...
    if (roiEntries.length === 0) {
        html += `<div style="font-size:9px;color:#e5e7eb;font-style:italic">Nessun dato ROI disponibile. Avvia una campagna per tracciare il ritorno sull'investimento.</div>`;
    } else {
