...
function renderTabFinance() {
    ...
    if (!document.getElementById('finance-css')) {
        ...
        s.id = 'finance-css';
        s.textContent = `
@keyframes fi-flash-up   { 0%{background:#0d2016} 100%{background:transparent} }
@keyframes fi-flash-down { 0%{background:#2d0d0d} 100%{background:transparent} }
.price-flash-up   { animation: fi-flash-up   .8s ease-out; }
.price-flash-down { animation: fi-flash-down .8s ease-out; }
.fi-risk-btn { background:#161b22;border:1px solid #21262d;border-radius:4px;padding:8px;cursor:pointer;text-align:left;flex:1;transition:border-color .15s; }
.fi-risk-btn.active { border-color:#b8962b; }
.fi-dur-btn  { background:#161b22;border:1px solid #21262d;border-radius:4px;padding:5px 8px;cursor:pointer;color:#8b949e;font-size:10px;flex:1;transition:border-color .15s; }
.fi-dur-btn.active  { border-color:#d4af37;color:#d4af37; }
...
.fi-credit-fill    { height:100%;border-radius:3px; }`;
    ...
    if (!hasWM) {
        ...
        <div style="background:#161b22;border:1px solid #b8962b;border-radius:6px;padding:28px;text-align:center">
            ...
            <div style="font-size:11px;color:#6b7280;margin-bottom:20px;max-width:340px;margin-left:auto;margin-right:auto">Assumi un <strong style="color:#e6edf3">Elite Wealth Manager</strong> nello Staff per sbloccare il mercato azionario, il broker personale e la leva finanziaria avanzata.</div>
    ...
    const plColor    = plPositive ? '#3fb950' : '#f85149';
        ...
        <tr id="ftick-${t.id}" style="border-bottom:1px solid #21262d">
            ...
            <td style="padding:8px 6px">
                ...
                <div style="display:flex;justify-content:space-between;font-size:9px;margin-bottom:3px">
                    ...
                    <span style="color:${plAmt >= 0 ? '#3fb950' : '#f85149'};font-weight:700">${plAmt >= 0 ? '+' : ''}€${plAmt.toLocaleString()} (${plPct.toFixed(1)}%)</span>
                </div>` : ''}
                ${shortPos ? `
                <div style="display:flex;justify-content:space-between;font-size:9px;margin-bottom:3px">
                    <span style="color:#a78bfa">Short: <b style="color:#e6edf3">${shortPos.shares}</b> @ €${shortPos.openPrice}</span>
                    <span style="color:${shortPl >= 0 ? '#3fb950' : '#f85149'};font-weight:700">${shortPl >= 0 ? '+' : ''}€${shortPl.toLocaleString()}</span>
                </div>` : ''}
                <div style="display:flex;gap:4px;margin-top:3px">
                    ...
                    ${holding.shares > 0 ? `<button onclick="sellStocks('${t.id}', parseInt(document.getElementById('stock-qty-${t.id}').value)||1)" style="background:#0d1a2e;border:1px solid #1a3a6b;border-radius:3px;padding:3px 7px;color:#58a6ff;font-size:9px;cursor:pointer">Vendi</button>` : ''}
                    <button onclick="shortSell('${t.id}', parseInt(document.getElementById('stock-qty-${t.id}').value)||1)" style="background:#2d0d0d;border:1px solid #6b2020;border-radius:3px;padding:3px 7px;color:#f85149;font-size:9px;cursor:pointer">Short↓</button>
                    ${shortPos ? `<button onclick="coverShort('${t.id}', parseInt(document.getElementById('stock-qty-${t.id}').value)||1)" style="background:#1a0d2e;border:1px solid #5a2d8b;border-radius:3px;padding:3px 7px;color:#a78bfa;font-size:9px;cursor:pointer">Copri</button>` : ''}
                </div>
            </td>
        </tr>`;
    });
    ...
    let brokerActiveHtml = brokerActive.length === 0
        ? `<div style="font-size:10px;color:#404040;font-style:italic;margin-bottom:12px">Nessun investimento attivo.</div>`
        : '';
    brokerActive.forEach(inv => {
        ...
        <div style="background:#0d1117;border:1px solid #21262d;border-radius:4px;padding:10px 12px;margin-bottom:8px">
            ...
            <div style="display:flex;justify-content:space-between;font-size:10px;margin-bottom:6px">
                <span style="color:#8b949e">Capitale: <b style="color:#e6edf3">€${inv.capital.toLocaleString()}</b></span>
                <span style="color:#8b949e">Rendimento: <b style="color:${profile?.color||'#e6edf3'}">${Math.round(inv.maxReturn*100)}% max</b></span>
            </div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
        ...
        <div style="text-align:right">
            ...
            <div style="font-size:9px;color:#3fb950">Disponibile: €${remainingCredit.toLocaleString()}</div>
        </div>
    </div>
    ...
    if (activeLoans.length > 0) {
        loansHtml += `<div style="font-size:9px;color:#f85149;font-weight:700;margin-bottom:8px">Prestiti attivi: €${activeLoanTotal.toLocaleString()}</div>`;
        activeLoans.forEach(l => {
            ...
            loansHtml += `<div style="background:#0d1117;border:1px solid #21262d;border-radius:4px;padding:6px 10px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;gap:8px">
                <span style="font-size:9px;color:#8b949e">Prestito #${l.id}</span>
                ...
                <button onclick="repayLoan(${l.id})" ${canRepay ? '' : 'disabled'}
                    style="background:#1a0808;border:1px solid #b8962b;border-radius:3px;padding:3px 8px;color:#d4af37;font-size:8px;cursor:${canRepay?'pointer':'default'};flex-shrink:0;opacity:${canRepay?1:0.4}">Salda</button>
            </div>`;
        });
    }
    ...
    const cempHtml = (() => {
        ...
        function _sparkCemp(h) {
            ...
            return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><polyline points="${pts}" fill="none" stroke="${cUp?'#3fb950':'#f85149'}" stroke-width="1.5" stroke-linecap="round"/></svg>`;
        }
        ...
        return `<div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:14px 16px;margin-bottom:12px">
            ...
            ${owned > 0 ? `<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px">
                ${[Math.min(50,owned), Math.min(owned, Math.floor(owned/2))].filter((v,i,a)=>a.indexOf(v)===i && v>0).map(qty => {
                    ...
                    return `<button onclick="sellCempShares(${qty})" style="background:#2d0d0d;border:1px solid #6b2020;border-radius:4px;padding:6px 4px;color:#f85149;font-size:9px;cursor:pointer;text-align:center;line-height:1.6">
                        Vendi ${qty}<br><span style="font-size:8px;opacity:.6">+€${rev.toLocaleString()}</span></button>`;
                }).join('')}
            </div>` : ''}
            <div style="font-size:8px;color:#404040;margin-top:8px;font-style:italic">Il prezzo $CEMP riflette la tua reputazione, gli incassi settimanali e la dimensione della flotta.</div>
        </div>`;
    })();
    ...
    const ipoHtml = (() => {
        ...
        if (!ipo?.listed) {
            return `<div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:14px 16px;margin-bottom:12px">
                ...
                <button onclick="listCompanyIPO()" ${canList ? '' : 'disabled'}
                    style="width:100%;background:${canList?'#1a1608':'#0d1117'};border:1px solid ${canList?'#b8962b':'#21262d'};border-radius:4px;padding:7px;color:${canList?'#d4af37':'#404040'};font-size:10px;cursor:${canList?'pointer':'not-allowed'};text-transform:uppercase;letter-spacing:.08em">
        ...
        return `<div style="background:#161b22;border:1px solid #1a3a20;border-radius:6px;padding:14px 16px;margin-bottom:12px">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
                <div>
            ...
            <div style="font-size:8px;color:#404040;margin-top:8px;font-style:italic">Ogni giorno il 10% degli utili viene distribuito agli azionisti NPC come dividendo.</div>
        </div>`;
    })();
    ...
    <div style="background:#161b22;border:1px solid #3d1a1a;border-radius:6px;padding:16px;text-align:center;margin-top:12px">
        ...
        <button onclick="sellCompanyNGP()" style="font-size:9px;border:1px solid #5a2020;color:#f85149;opacity:.7;background:none;border-radius:3px;padding:4px 12px;cursor:pointer">Avvia Exit Strategy ↗</button>
    </div>`;
