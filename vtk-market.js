        ...
        <div style="padding:16px 20px;border-bottom:1px solid #21262d;display:flex;align-items:center;justify-content:space-between">
            ...
            <div style="display:flex;align-items:center;gap:16px">
                <div style="text-align:center">
                    ...
                    <div style="font-size:16px;font-weight:700;color:#58a6ff;font-family:monospace">◈ ${vtk}</div>
                </div>
                <div style="text-align:center">
                    <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.06em">DC</div>
                    <div style="font-size:16px;font-weight:700;color:#d4af37;font-family:monospace">🪙 ${dc}</div>
                </div>
                <button onclick="document.getElementById('vtk-modal').remove()"
                    style="background:transparent;border:1px solid #21262d;color:#8b949e;border-radius:4px;padding:4px 8px;cursor:pointer;font-size:14px;line-height:1">✕</button>
            </div>
        </div>
        ...
        <div style="display:flex;gap:0;border-bottom:1px solid #21262d">
            <button onclick="window._vtkState._subTab='market';window.renderVTKModal()"
                style="flex:1;padding:10px;font-size:10px;font-weight:600;cursor:pointer;border:none;transition:all .15s;
                    ${subTab==='market' ? 'background:#1a1a2a;color:#58a6ff;border-bottom:2px solid #58a6ff' : 'background:transparent;color:#8b949e;border-bottom:2px solid transparent'}">
                📈 Mercato P2P
            </button>
            <button onclick="window._vtkState._subTab='shop';window.renderVTKModal()"
                style="flex:1;padding:10px;font-size:10px;font-weight:600;cursor:pointer;border:none;transition:all .15s;
                    ${subTab==='shop' ? 'background:#1a1a2a;color:#d4af37;border-bottom:2px solid #d4af37' : 'background:transparent;color:#8b949e;border-bottom:2px solid transparent'}">
                🛒 VTK Shop
            </button>
        </div>
...
function _vtkRenderMarket(vtk, dc, myOrders, otherOrders) {
    ...
    <div style="background:#0d1117;border:1px solid #21262d;border-radius:6px;padding:14px;margin-bottom:16px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
            <div>
                <div style="font-size:9px;color:#8b949e;margin-bottom:4px">VTK da vendere</div>
            ...
            <div>
                <div style="font-size:9px;color:#8b949e;margin-bottom:4px">Prezzo (DC totale)</div>
        ...
        <button onclick="window.vtkPlaceSellOrder(document.getElementById('vtk-sell-amount').value, document.getElementById('vtk-sell-price').value)"
            style="width:100%;padding:7px;font-size:10px;font-weight:700;cursor:pointer;background:#0d1b2a;border:1px solid rgba(88,166,255,0.4);color:#58a6ff;border-radius:4px;transition:opacity .15s"
    ...
    <div style="margin-bottom:16px">
        ...
        <div style="background:#161b22;border:1px solid rgba(212,175,55,0.2);border-radius:6px;padding:10px 12px;margin-bottom:6px;display:flex;align-items:center;justify-content:space-between">
            <div style="font-size:11px;color:#e6edf3">
                <span style="color:#58a6ff;font-family:monospace">◈ ${o.vtk_amount} VTK</span>
                <span style="color:#6b7280;margin:0 8px">→</span>
                <span style="color:#d4af37;font-family:monospace">🪙 ${o.dc_price} DC</span>
            </div>
            <button onclick="window.vtkCancelOrder('${o.id}')"
                style="font-size:9px;padding:3px 8px;cursor:pointer;background:#2d0d0d;border:1px solid #5a1a1a;color:#f85149;border-radius:4px">
                Annulla
            </button>
        </div>`).join('')}
    </div>` : ''}
    ...
    <div>
        ...
        ${otherOrders.length === 0
            ? `<div style="text-align:center;padding:24px 0;font-size:10px;color:#8b949e;font-style:italic">Nessun ordine disponibile al momento.</div>`
            : otherOrders.map(o => `
            <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px;margin-bottom:6px;display:flex;align-items:center;justify-content:space-between">
                ...
                <button onclick="window.vtkFillOrder('${o.id}', ${o.dc_price})"
                    ...
                    style="font-size:9px;padding:5px 12px;cursor:${dc < o.dc_price ? 'not-allowed' : 'pointer'};background:#1a1608;border:1px solid #b8962b;color:#d4af37;border-radius:4px;${dc < o.dc_price ? 'opacity:.4' : ''};transition:opacity .15s"
...
function _vtkRenderShop(vtk) {
    return `
    <div style="background:rgba(212,175,55,0.04);border:1px solid rgba(212,175,55,0.15);border-radius:6px;padding:12px;margin-bottom:16px;font-size:10px;color:#d4af37;line-height:1.5">
    ...
    ${VTK_SHOP_ITEMS.map(item => {
        ...
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:14px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between;gap:12px">
            <div style="flex:1">
                ...
                <div style="font-size:10px;color:#8b949e;margin-left:28px">${item.desc}</div>
            </div>
            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0">
                <div style="font-size:12px;font-weight:700;color:#58a6ff;font-family:monospace">◈ ${item.cost}</div>
                <button onclick="window.vtkBuyShopItem('${item.id}')"
                    ${canBuy ? '' : 'disabled'}
                    style="padding:5px 12px;font-size:9px;font-weight:700;border-radius:4px;cursor:${canBuy?'pointer':'not-allowed'};background:${canBuy?'#0d1b2a':'#161b22'};border:1px solid ${canBuy?'rgba(88,166,255,0.4)':'#21262d'};color:${canBuy?'#58a6ff':'#6b7280'};${canBuy?'':'opacity:.5'};transition:opacity .15s"
                    onmousedown="if(!this.disabled)this.style.transform='scale(0.97)'" onmouseup="this.style.transform=''" onmouseleave="this.style.transform=''">
                    Acquista
                </button>
            </div>
