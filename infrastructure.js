...
window._infraBuyDepot = async function(provinceId, provinceName) {
    ...
    if ((window.gameState?.cash || 0) < cost) {
        if (typeof showNotification === 'function') showNotification(`Fondi insufficienti! Servono €${cost.toLocaleString('it-IT')}`, 'error');
        return;
    }
    ...
    try {
        ...
        if (window.CE_money) window.CE_money.addebitatoDalServer(cost, 'buy_fuel_depot');
