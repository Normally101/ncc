...
test('generate AZIONI-interfaccia.md', () => {
    const targetFiles = [
        'ui-career.js',
        'ui-dispatch.js',
        'ui-emails.js',
    ...
    for (const tf of targetFiles) {
        ...
        // Filter duplicates defined in the same file (e.g. function renderTabStaff ... window.renderTabStaff = renderTabStaff)
        const seenNamesInFile = new Set();
        const uniqueDefs = [];
        for (const d of defs) {
            if (seenNamesInFile.has(d.name)) continue;
            seenNamesInFile.add(d.name);
            uniqueDefs.push(d);
        }
        ...
        for (const d of uniqueDefs) {
            ...
            // Duplicates across other files
            const allOfName = projectDefs.get(d.name) || [];
            const otherDefs = allOfName.filter(x => x.file !== tf);
            if (otherDefs.length > 0) {
                const exists = duplicates.some(x => x.name === d.name && (x.f1.startsWith(tf) || x.f2.includes(tf)));
                if (!exists) {
    ...
    for (const dup of duplicates) {
        let note = '';
        if (dup.name === 'renderTabProvinces') note = 'ui-ops.js e war_room.js definivano la stessa tab; risolto separando in renderTabWarRoom';
        else if (dup.name === 'switchTab' || dup.name === 'updateUI') note = 'Decoratore / wrapper globale dell\'interfaccia';
        else if (dup.name === '_kpi') note = 'Helper locale duplicato in scope globale (ui-finance.js e ui-ranking.js)';
    ...
    md += '\n---\n\n';
    md += '## Segnalazione speciale: Configuratore Auto in `ui-staff.js`\n\n';
    md += 'Il configuratore auto in `ui-staff.js` (righe ~400–585, circa 185 righe) comprende le funzioni:\n';
    md += '- `openCarConfigurator` (ui-staff.js:400)\n';
    md += '- `_updateSummary` (ui-staff.js:490)\n';
    md += '- `__cfgToggle` (ui-staff.js:516)\n';
    md += '- `__cfgConfirm` (ui-staff.js:537)\n';
    md += '- `buyCar` (ui-staff.js:583)\n';
    md += '- `leaseCar` (ui-staff.js:585)\n\n';
    md += 'Questo blocco era stato identificato come codice orfano/morto nell\'analisi del 19/08/2026 poichè l\'acquisto veicoli primario è gestito da `showroom.js` (`renderTabShowroom`, `_srmPurchase`), mentre la vendita/gestione flotta è in `engine-fleet.js` e `p2p-market.js`.\n';

    fs.writeFileSync('docs/AZIONI-interfaccia.md', md, 'utf8');
    console.log('Successfully written docs/AZIONI-interfaccia.md');
});
