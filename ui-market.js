...
function renderTabMarket() {
    ...
    const _btn  = (t, fn, c, dis) => {
        ...
        const bd = c==='gold'?'#b8962b':c==='red'?'#471a1a':'#21262d';
        const tc = c==='gold'?'#d4af37':c==='red'?'#f85149':'#8b949e';
...
<div style="padding:16px;max-width:800px">
    ...
    <div style="padding-bottom:16px;border-bottom:1px solid #21262d;margin-bottom:16px">
        ...
        <div style="font-size:11px;color:#8b949e;margin-top:2px">${npcList.length} disponibili · ${myListings.length} tuoi annunci · Flotta stimata €${Math.round(fleetVal/1000)}k</div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:20px">
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            ...
            <div style="font-size:18px;font-weight:700;color:${npcList.length>0?'#58a6ff':'#e6edf3'};font-family:monospace">${npcList.length}</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Tuoi Annunci</div>
            <div style="font-size:18px;font-weight:700;color:${myListings.length>0?'#d4af37':'#e6edf3'};font-family:monospace">${myListings.length}</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Asta Live</div>
            <div style="font-size:18px;font-weight:700;color:${auc?'#f85149':'#e6edf3'};font-family:monospace">${auc?'ATTIVA':'—'}</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:6px;padding:10px 12px">
            <div style="font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Budget</div>
            <div style="font-size:18px;font-weight:700;color:#3fb950;font-family:monospace">€${((gameState.cash||0)/1000).toFixed(0)}k</div>
        </div>
    </div>`;
    ...
    if (auc) {
        ...
        const urgColor  = hoursLeft < 3 ? '#f85149' : '#e6edf3';
        const bids      = [auc.currentBid + 5000, auc.currentBid + 15000, auc.currentBid + 50000];
        html += `
        <div style="background:#161b22;border:1px solid #b8962b;border-radius:6px;padding:16px;margin-bottom:4px">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px">
                <div>
                    <div style="font-size:13px;font-weight:700;color:#e6edf3">${auc.name}</div>
                    <div style="font-size:10px;color:#8b949e;margin-top:2px;font-family:monospace">${auc.tier.toUpperCase()}</div>
                    <div style="margin-top:8px">
                        ${isWinning ? _pill('STAI VINCENDO', '#3fb950') : auc.playerBid ? _pill('SUPERATO', '#f85149') : _pill('FAI UN\'OFFERTA', '#58a6ff')}
                    </div>
                </div>
                <div style="text-align:right">
                    ...
                    <div style="font-size:20px;font-weight:700;font-family:monospace;color:#d4af37">€${auc.currentBid.toLocaleString()}</div>
                </div>
            </div>
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
                ${bids.map(bid => `
                <button onclick="bidOnAuction(${bid})" style="background:#1a1608;border:1px solid #b8962b;color:#d4af37;padding:8px 6px;border-radius:4px;cursor:pointer;font-family:monospace;font-weight:700;font-size:9px;display:flex;flex-direction:column;align-items:center;gap:2px">
        ...
        html += `<div style="text-align:center;padding:20px;background:#161b22;border:1px solid #21262d;border-radius:6px;margin-bottom:4px">
            <div style="font-size:13px;color:#8b949e;margin-bottom:4px">Nessuna asta attiva</div>
    ...
    if (npcList.length === 0) {
        ...
        npcList.forEach(l => {
            ...
            const condColor = cond < 40 ? '#f85149' : cond < 70 ? '#f59e0b' : '#3fb950';
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid #21262d;gap:12px">
                <div style="flex:1;min-width:0">
                    ...
                    <div style="font-size:10px;color:#8b949e;margin-top:2px;font-family:monospace">${l.tier.toUpperCase()} · ${Math.floor(l.mileage/1000)}k km · <span style="color:${condColor}">${cond}%</span></div>
                </div>
                <div style="display:flex;align-items:center;gap:10px;flex-shrink:0">
                    <div style="font-size:14px;font-weight:700;color:#d4af37;font-family:monospace">€${l.price.toLocaleString()}</div>
    ...
    if (myListings.length === 0) {
        html += `<div style="font-size:11px;color:#8b949e;padding:12px 0">Nessun annuncio attivo. Vai in <strong style="color:#e6edf3">Flotta</strong> → card veicolo → Metti in Vendita.</div>`;
    } else {
        ...
        myListings.forEach(l => {
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid #21262d;gap:12px">
                <div style="flex:1;min-width:0">
                    ...
                    <div style="font-size:10px;color:#8b949e;margin-top:2px;font-family:monospace">€${l.askPrice.toLocaleString()} · ${daysLeft > 0 ? `Acquirente ~${daysLeft}g` : 'Vendita in corso…'}</div>
                </div>
        ...
        sellable.forEach(car => {
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid #21262d;gap:12px">
                <div style="flex:1;min-width:0">
                    ...
                    <div style="font-size:10px;color:#8b949e;margin-top:2px;font-family:monospace">${car.tier.toUpperCase()} · ${condPct}% · Stima <span style="color:#3fb950">~€${suggest.toLocaleString()}</span></div>
                </div>
