-- ============================================================
-- 33_tourism_tenders.sql — Chauffeur Empire
-- Dynamic B2B Tender System: 21 Luxury Tourism Companies
-- PvP rotating contracts · competitive bidding · passive income
-- ============================================================

-- ── TABLES ───────────────────────────────────────────────────────────────────

-- Static catalog of the 21 tourism companies (never changes)
CREATE TABLE IF NOT EXISTS public.b2b_catalog (
    ...
    -- requirements: {min_reputation, req_tier, req_vehicle_count, req_driver_count, req_completions}
    requirements         JSONB       NOT NULL DEFAULT '{}'::JSONB,
    created_at           TIMESTAMPTZ DEFAULT NOW()
);

-- Live tender state — one row per catalog entry per round
-- Cycles: open_bidding → active → cooldown → open_bidding
...
-- Per-player bids on an open tender
...
-- ── SEED: b2b_catalog (21 luxury tourism companies) ──────────────────────────
...
VALUES
  ...
  ('Imperial Silk Routes',
   ...
   4, '🏯', 'Puntualità assoluta, sedili executive, discrezione diplomatica. Zero tollleranza ritardi.',
   260, 2.0, 14,
...
-- ── INITIALIZE b2b_active_tenders (one row per catalog entry) ────────────────
...
-- ── INTERNAL: process tender lifecycle ───────────────────────────────────────
-- Called automatically from rpc_get_tourism_tenders (self-healing).
...
DECLARE
    ...
    v_daily  BIGINT;
BEGIN
    -- 1. Award tenders where bidding window has closed
    FOR v_tender IN
    ...
    LOOP
        ...
        IF FOUND THEN
            ...
            -- Deduct pledge from winner's cash (signing fee)
            ...
            WHERE  tender_id = v_tender.id AND status = 'pending';
        ELSE
            -- No bids: extend window another 24h
    ...
    -- 2. Expire active contracts past their end date
    FOR v_tender IN
    ...
    -- 3. Re-open tenders after cooldown
    UPDATE public.b2b_active_tenders SET
        status          = 'open_bidding',
...
-- ── RPC: rpc_get_tourism_tenders ─────────────────────────────────────────────
-- Returns all tenders + catalog info + player's own bid data.
-- Also triggers _process_tourism_tenders (self-healing lifecycle).
...
-- ── RPC: rpc_submit_tourism_bid ───────────────────────────────────────────────
-- Score = reputation(40%) + fleet(40%) + pledge(20%)
-- Pledge NOT deducted now; taken at award time from winner only.
...
-- ── RPC: rpc_tourism_daily_tick ───────────────────────────────────────────────
-- Pay all active tourism contracts owned by calling user.
-- Called from engine.js processDailyRoutines alongside _b2bDailyTick.
...
BEGIN
    ...
    FOR v_tender IN
        ...
        FOR UPDATE OF bat SKIP LOCKED
    LOOP
        -- Only pay if real-time threshold passed
...
-- ── RPC: rpc_terminate_tourism_contract ──────────────────────────────────────
-- Early termination: rep penalty, no cash refund.
...
BEGIN
    ...
    -- Rep penalty proportional to tier
    UPDATE public.companies
...
-- ── RPC: rpc_expire_tourism_contracts ────────────────────────────────────────
-- Standalone expiration function for pg_cron scheduling.
-- Cron setup (run in Supabase SQL editor if pg_cron is enabled):
--   SELECT cron.schedule('tourism-lifecycle', '0 * * * *', 'SELECT public._process_tourism_tenders()');
