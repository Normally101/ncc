...
--
-- AGGIUNGE:
--   1. tabella cheat_flags  — log anomalie server-side
--   2. rate-limit listing   — max 5 inserzioni attive per utente
--   3. sanity prezzo listing — €1.000 ≤ ask_price ≤ €50.000.000
--   4. rate-limit bid        — max 1 aggiornamento offerta per 10s (per auction+user)
--   5. flag automatico       — listing > €10M o bid > €20M loggati in cheat_flags
--   6. rpc_list_car_for_sale v2 — include tutti i check
--   7. rpc_place_auction_bid v2 — include rate-limit + flag
--
-- Idempotente. Eseguire UNA VOLTA su Supabase SQL Editor.
-- ════════════════════════════════════════════════════════════════════════

-- ── 1. TABELLA CHEAT FLAGS ────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.cheat_flags (
    id          BIGSERIAL   PRIMARY KEY,
    user_id     UUID        REFERENCES auth.users(id) ON DELETE SET NULL,
    flag_type   TEXT        NOT NULL,   -- 'listing_price', 'bid_spike', 'rate_limit', ecc.
    details     JSONB       DEFAULT '{}',
    created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);
ALTER TABLE public.cheat_flags ENABLE ROW LEVEL SECURITY;
-- solo admin può leggere; il trigger server scrive senza RLS
...
-- helper interno: inserisce flag (SECURITY DEFINER → bypassa RLS)
CREATE OR REPLACE FUNCTION public._flag_cheat(p_uid uuid, p_type text, p_details jsonb)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  INSERT INTO public.cheat_flags(user_id, flag_type, details) VALUES (p_uid, p_type, p_details);
EXCEPTION WHEN OTHERS THEN NULL; -- non bloccare mai l'operazione principale
END;
$$;

-- ── 2. INDICE bid-timestamp (per rate-limit offerte) ─────────────────────────
ALTER TABLE public.judicial_bids ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT now();

-- ── 3. rpc_list_car_for_sale v2 — rate-limit + sanity prezzo ─────────────────
...
BEGIN
    ...
    -- Sanity prezzo (server-authoritative: il client non può proporre prezzi assurdi)
    ...
    -- Rate-limit: max 5 inserzioni attive per utente
    ...
    -- Flag anomalia prezzo elevato (non blocca, solo segnala)
...
-- ── 4. rpc_place_auction_bid v2 — rate-limit 10s + flag spike ────────────────
...
BEGIN
    ...
    -- Rate-limit: 1 aggiornamento ogni 10s per (utente, asta)
    ...
    IF v_prev_ts IS NOT NULL AND now() - v_prev_ts < INTERVAL '10 seconds' THEN
        PERFORM public._flag_cheat(v_uid, 'bid_rate_limit',
            jsonb_build_object('auction_id', v_auction_id, 'interval_secs',
                EXTRACT(EPOCH FROM (now() - v_prev_ts))));
